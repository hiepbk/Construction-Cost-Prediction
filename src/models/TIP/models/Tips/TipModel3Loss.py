...
class TIP3Loss(Pretraining):
    ...
    def training_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor], Optional[torch.Tensor], Optional[list]], _) -> torch.Tensor:
        ...
        self.log(f"multimodal.train.ITCloss", loss_itc, on_epoch=True, on_step=False, sync_dist=True)

        # =======================================  itm  =======================================================================
        loss_itm, logits_itm, labels_itm = self.cal_image_tabular_matching_loss(image_embeddings, tabular_embeddings, logits)
        self.log(f"multimodal.train.ITMloss", loss_itm, on_epoch=True, on_step=False, sync_dist=True)
        ...
        self.log(f"multimodal.train.TRloss", loss_tr, on_epoch=True, on_step=False, sync_dist=True)
        ...
        self.log(f"multimodal.train.loss", loss, on_epoch=True, on_step=False, sync_dist=True)
        return {'loss':loss, 'embeddings': multimodal_embeddings, 'labels': y, }

    
    def validation_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor], Optional[torch.Tensor], Optional[list]], _) -> torch.Tensor:
        '''
        Validate
        Tabular-imaging contrastive learning
        Tabular reconstruction learning
        ...
        self.log(f"multimodal.val.ITCloss", loss_itc, on_epoch=True, on_step=False, sync_dist=True)

        # =======================================  itm  =======================================================================
        loss_itm, logits_itm, labels_itm = self.cal_image_tabular_matching_loss(image_embeddings, tabular_embeddings, logits)
        self.log(f"multimodal.val.ITMloss", loss_itm, on_epoch=True, on_step=False, sync_dist=True)
        ...
        z2, multimodal_embeddings = self.forward_multimodal(tabular_features=tabular_embeddings, image_features=image_embeddings)
        loss_tr, pred_cat, target_cat, mask_cat = self.criterion_tr(z2,original_tab, mask=mask)
        self.log(f"multimodal.val.TRloss", loss_tr, on_epoch=True, on_step=False, sync_dist=True)
        ...
        loss = (loss_itc + loss_tr + loss_itm)/3.0
        # loss = (loss_itc + loss_tr)/3.0
        self.log(f"multimodal.val.loss", loss, on_epoch=True, on_step=False, sync_dist=True)
