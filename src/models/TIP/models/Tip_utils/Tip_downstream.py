...
class TIPBackbone(nn.Module):
  ...
  def __init__(self, args, checkpoint_dict=None) -> None:
    ...
    if args.checkpoint or checkpoint_dict:
      ...
      elif original_args.model == 'satlas' or (hasattr(original_args, 'use_satellite_encoder') and original_args.use_satellite_encoder):
        # Handle satellite encoder (SatlasPretrain)
        from models.Tip_utils.SatelliteImageEncoder import create_satellite_image_encoder
        self.encoder_imaging = create_satellite_image_encoder(original_args)
      ...
      # Set default finetune_strategy if not provided
      finetune_strategy = getattr(args, 'finetune_strategy', 'trainable')
  ...
  def create_imaging_model(self, args):
    ...
    elif args.model == 'satlas' or (hasattr(args, 'use_satellite_encoder') and args.use_satellite_encoder):
      # Handle satellite encoder (SatlasPretrain)
      from models.Tip_utils.SatelliteImageEncoder import create_satellite_image_encoder
      self.encoder_imaging = create_satellite_image_encoder(args)
  ...
  def forward(self, x: torch.Tensor, visualize=False) -> torch.Tensor:
    ...
    # Process image
    img_features = self.encoder_imaging(x_i)[-1]   # (B,C,H,W) or (B,C) for satellite
    
    # Handle different encoder types
    import torch.nn.functional as F
    if hasattr(self, 'encoder_imaging_type') and self.encoder_imaging_type == 'satellite':
      # Satellite encoder already returns pooled features
      if img_features.dim() == 4:
        # If still spatial, pool it
        x_i = F.adaptive_avg_pool2d(img_features, (1, 1)).flatten(1)
      else:
        x_i = img_features
    else:
      # ResNet/ViT: need to pool
      if img_features.dim() == 4:
        x_i = F.adaptive_avg_pool2d(img_features, (1, 1)).flatten(1)
    ...
    # Add sequence dimension for multimodal encoder if needed
    # The multimodal encoder expects image_features to have shape (B, N_k, K) or (B, C, H, W)
    # If we have 2D tensor (B, C), add sequence dimension: (B, C) -> (B, 1, C)
    if x_i.dim() == 2:
      x_i = x_i.unsqueeze(1)  # (B, C) -> (B, 1, C)
