...
class Pretraining(pl.LightningModule):
  ...
  def initialize_imaging_encoder_and_projector(self) -> None:
    """
    Selects appropriate image encoder (ResNet, ViT, or Satellite encoder)
    """
    # Check if using satellite encoder
    if hasattr(self.hparams, 'use_satellite_encoder') and self.hparams.use_satellite_encoder:
      from models.Tip_utils.SatelliteImageEncoder import create_satellite_image_encoder
      self.encoder_imaging_type = 'satellite'
      self.encoder_imaging = create_satellite_image_encoder(self.hparams)
      # Get pooled dimension from encoder
      if hasattr(self.encoder_imaging, 'pooled_dim'):
        self.pooled_dim = self.encoder_imaging.pooled_dim
      else:
        # Default for Satlas: 2 * feature_dim
        self.pooled_dim = getattr(self.hparams, 'satellite_feature_dim', 512) * 2
    elif self.hparams.model.startswith('vit'):
  ...
  def initialize_multimodal_encoder_and_predictor(self) -> None:
    # Set image encoder output dimension for multimodal encoder
    # Use pooled_dim (actual encoder output) instead of embedding_dim (projection head output)
    with open_dict(self.hparams):
      self.hparams.image_encoder_output_dim = self.pooled_dim
  ...
  def initialize_classifier_and_metrics(self, nclasses_train, nclasses_val):
    ...
    if self.hparams.num_classes == 1:
      # Regression task - skip ALL classification metrics (they're not used for regression)
      self.classifier_acc_train = None
      self.classifier_acc_val = None
      self.classifier_auc_train = None
      self.classifier_auc_val = None
      # Skip contrastive learning metrics (not used for regression)
      self.top1_acc_train = None
      self.top1_acc_val = None
      self.top5_acc_train = None
      self.top5_acc_val = None
      # Skip categorical reconstruction metrics (not used for regression)
      self.top1_acc_train_cat = None
      self.top5_acc_train_cat = None
      self.top1_acc_val_cat = None
      self.top5_acc_val_cat = None
      self.auc_train_cat = None
      self.auc_val_cal = None
      # Skip ITM metrics (not used for regression)
      self.acc_train_itm = None
      self.acc_val_itm = None
    else:
      # Classification task - initialize all metrics
  ...
  def forward_imaging(self, x: torch.Tensor) -> torch.Tensor:
    ...
    if self.encoder_imaging_type == 'resnet':
      ...
      z = y[:,0,:]
    elif self.encoder_imaging_type == 'satellite':
      # Satellite encoder already returns pooled features
      if y.dim() == 4:
        # If still spatial, pool it
        z = F.adaptive_avg_pool2d(y, (1, 1)).flatten(1)
      else:
        # Already flattened
        z = y
    else:
      raise ValueError(f"Unknown encoder type: {self.encoder_imaging_type}")
  ...
  def calc_and_log_train_embedding_acc(self, logits, labels, modality: str) -> None:
    ...
    if self.hparams.num_classes == 1 or self.top1_acc_train is None:
      return
  ...
  def calc_and_log_val_embedding_acc(self, logits, labels, modality: str) -> None:
    # Skip for regression tasks
    if self.hparams.num_classes == 1 or self.top1_acc_val is None:
      return
  ...
  def calc_and_log_train_cat_embedding_acc(self, logits, labels, mask, modality: str) -> None:
    # Skip for regression tasks
    if self.hparams.num_classes == 1 or self.top1_acc_train_cat is None:
      return
  ...
  def calc_and_log_val_cat_embedding_acc(self, logits, labels, mask, modality: str) -> None:
    # Skip for regression tasks
    if self.hparams.num_classes == 1 or self.top1_acc_val_cat is None:
      return
    logits, labels = logits[mask].detach(), labels[mask].detach()
  ...
  def calc_and_log_train_itm_acc(self, logits, labels, modality: str) -> None:
    # Skip for regression tasks
    if self.hparams.num_classes == 1 or self.acc_train_itm is None:
      return
  ...
  def calc_and_log_val_itm_acc(self, logits, labels, modality: str) -> None:
    # Skip for regression tasks
    if self.hparams.num_classes == 1 or self.acc_val_itm is None:
      return
    logits, labels = logits.detach(), labels.detach()
  ...
  def training_epoch_end(self, train_step_outputs: List[Any]) -> None:
    ...
    """
    # Skip classifier evaluation for regression tasks (num_classes=1)
    if self.hparams.num_classes == 1:
      return
