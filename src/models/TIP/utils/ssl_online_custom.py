...
class SSLOnlineEvaluator(Callback):  # pragma: no cover
    ...
    def shared_step(
        ...
        # torchmetrics 0.9.3 API: auroc and accuracy handle both binary and multiclass
        if self.num_classes == 2:
          # Binary: use positive class probabilities
          auc = auroc(mlp_logits_sm[:, 1], y)
          acc = accuracy(mlp_logits_sm.argmax(dim=1), y)
        else:
          # Multiclass: use all class probabilities
          auc = auroc(mlp_logits_sm, y, num_classes=self.num_classes)
          acc = accuracy(mlp_logits_sm.argmax(dim=1), y, num_classes=self.num_classes)
