...
class Weights:
    def __init__(self, pretrained_dir=None):
        ...
        Args:
            pretrained_dir: Directory to save/load pretrained weights. 
                          If None, uses 'pretrained' folder in project root.
        """
        super(Weights, self).__init__()
        
        # Determine pretrained directory
        if pretrained_dir is None:
            # Try to find project root (look for 'pretrained' folder)
            current_file = Path(__file__).resolve()
            # Go up from satlaspretrain_models/satlaspretrain_models/model.py
            # to project root
            project_root = current_file.parent.parent.parent.parent.parent
            self.pretrained_dir = project_root / 'pretrained'
        else:
    ...
    def _get_weights_path(self, weights_url):
        """Extract filename from URL and return local path"""
        # Parse URL to get filename
        parsed = urlparse(weights_url)
        # Remove query parameters and get filename
        filename = os.path.basename(unquote(parsed.path))
        # Remove ?download=true if present in filename
        if '?' in filename:
            filename = filename.split('?')[0]
        return self.pretrained_dir / filename

    def _download_weights(self, weights_url, weights_path):
        """Download weights from URL and save to local path"""
        ...
        print(f"\n✓ Successfully downloaded to {weights_path}")
    ...
    def get_pretrained_model(self, model_identifier, fpn=False, head=None, num_categories=None, device='cuda'):
        ...
        # Get weights URL and local path
        weights_url = model_info['url']
        weights_path = self._get_weights_path(weights_url)
        
        # Download if not exists
        if not weights_path.exists():
            self._download_weights(weights_url, weights_path)
        else: 
